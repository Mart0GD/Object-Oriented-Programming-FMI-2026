...
2) Ако напишем дефиницията на функцията някъде другате, във header файл или в клас, стойностт по подразбиране се пиша само в декларацията на функцията, а не и в дефиницията. Това ще доведе до компилационна грешка!
...
int main(void)
{
    ...
    // НЕВАЛИДНО, това е референция без стойност, при опит за достъпване ще се хвъли грешка
    int& ref;

    // НЕВАЛИДНО, това е dangling reference, тъй като 
    int& ref = return_2();
}
